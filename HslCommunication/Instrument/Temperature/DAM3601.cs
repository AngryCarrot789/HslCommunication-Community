...
public class DAM3601 : ModbusRtu {
    ...
    public OperateResult<float[]> ReadAllTemperature() {
        ...
        OperateResult<short[]> read = this.ReadInt16(address, 128);
        if (!read.IsSuccess)
            return OperateResult.CreateFailedResult<float[]>(read);

        return OperateResult.CreateSuccessResult(read.Content.Select(m => this.TransformValue(m)).ToArray());
    }
    ...
    public override OperateResult<byte[]> Read(string address, ushort length) {
        OperateResult<ModbusAddress> analysis = ModbusInfo.AnalysisAddress(address, this.AddressStartWithZero, ModbusInfo.ReadRegister);
