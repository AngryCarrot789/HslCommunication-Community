...
public class SiemensS7Server : NetworkDataServerBase {
    ...
    public SiemensS7Server() {
        ...
        this.outputBuffer = new SoftBuffer(DataPoolLength);
        this.memeryBuffer = new SoftBuffer(DataPoolLength);
        this.dbBlockBuffer = new SoftBuffer(DataPoolLength);
    ...
    public override OperateResult<byte[]> Read(string address, ushort length) {
        ...
        switch (analysis.Content.DataCode) {
            case 0x81: return OperateResult.CreateSuccessResult(this.inputBuffer.GetBytes(analysis.Content.AddressStart / 8, length));
            case 0x82: return OperateResult.CreateSuccessResult(this.outputBuffer.GetBytes(analysis.Content.AddressStart / 8, length));
            case 0x83: return OperateResult.CreateSuccessResult(this.memeryBuffer.GetBytes(analysis.Content.AddressStart / 8, length));
            case 0x84: return OperateResult.CreateSuccessResult(this.dbBlockBuffer.GetBytes(analysis.Content.AddressStart / 8, length));
    ...
    public override OperateResult Write(string address, byte[] value) {
        ...
        switch (analysis.Content.DataCode) {
            case 0x81:
                this.inputBuffer.SetBytes(value, analysis.Content.AddressStart / 8);
                return OperateResult.CreateSuccessResult();
            case 0x82:
                this.outputBuffer.SetBytes(value, analysis.Content.AddressStart / 8);
                return OperateResult.CreateSuccessResult();
            case 0x83:
                this.memeryBuffer.SetBytes(value, analysis.Content.AddressStart / 8);
                return OperateResult.CreateSuccessResult();
            case 0x84:
                this.dbBlockBuffer.SetBytes(value, analysis.Content.AddressStart / 8);
                return OperateResult.CreateSuccessResult();
            default: return new OperateResult<byte[]>(StringResources.Language.NotSupportedDataType);
        }
    }
    ...
    public OperateResult<bool> ReadBool(string address) {
        ...
        switch (analysis.Content.DataCode) {
            case 0x81: return OperateResult.CreateSuccessResult(this.inputBuffer.GetBool(analysis.Content.AddressStart));
            case 0x82: return OperateResult.CreateSuccessResult(this.outputBuffer.GetBool(analysis.Content.AddressStart));
            case 0x83: return OperateResult.CreateSuccessResult(this.memeryBuffer.GetBool(analysis.Content.AddressStart));
            case 0x84: return OperateResult.CreateSuccessResult(this.dbBlockBuffer.GetBool(analysis.Content.AddressStart));
    ...
    public OperateResult Write(string address, bool value) {
        ...
        switch (analysis.Content.DataCode) {
            case 0x81:
                this.inputBuffer.SetBool(value, analysis.Content.AddressStart);
                return OperateResult.CreateSuccessResult();
            case 0x82:
                this.outputBuffer.SetBool(value, analysis.Content.AddressStart);
                return OperateResult.CreateSuccessResult();
            case 0x83:
                this.memeryBuffer.SetBool(value, analysis.Content.AddressStart);
                return OperateResult.CreateSuccessResult();
            case 0x84:
                this.dbBlockBuffer.SetBool(value, analysis.Content.AddressStart);
                return OperateResult.CreateSuccessResult();
    ...
    protected override void ThreadPoolLoginAfterClientCheck(Socket socket, System.Net.IPEndPoint endPoint) {
        ...
        OperateResult<byte[]> read1 = this.ReceiveByMessage(socket, 5000, s7Message);
        if (!read1.IsSuccess)
            return;

        OperateResult send1 = this.Send(socket, SoftBasic.HexStringToBytes("03 00 00 16 11 D0 00 01 00 0C 00 C0 01 0A C1 02 01 02 C2 02 01 00"));
        if (!send1.IsSuccess)
            return;

        OperateResult<byte[]> read2 = this.ReceiveByMessage(socket, 5000, s7Message);
        if (!read1.IsSuccess)
            return;

        OperateResult send2 = this.Send(socket, SoftBasic.HexStringToBytes("03 00 00 1B 02 F0 80 32 03 00 00 04 00 00 08 00 00 00 00 F0 00 00 01 00 01 00 F0"));
        if (!read2.IsSuccess)
            return;
    ...
    private void SocketAsyncCallBack(IAsyncResult ar) {
        if (ar.AsyncState is AppSession session) {
            try {
                ...
                S7Message s7Message = new S7Message();
                OperateResult<byte[]> read1 = this.ReceiveByMessage(session.WorkSocket, 5000, s7Message);
                if (!read1.IsSuccess) {
                ...
                if (receive[17] == 0x04) {
                    // 读数据
                    session.WorkSocket.Send(this.ReadByMessage(receive));
                }
                else if (receive[17] == 0x05) {
                    // 写数据
                    session.WorkSocket.Send(this.WriteByMessage(receive));
                }
    ...
    private byte[] ReadByMessage(byte[] packCommand) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            byte[] command = this.ByteTransform.TransByte(packCommand, index, length + 2);
            index += length + 2;

            content.AddRange(this.ReadByCommand(command));
        }
    ...
    private byte[] ReadByCommand(byte[] command) {
        if (command[3] == 0x01) {
            ...
            switch (command[8]) {
                case 0x81: return this.PackReadBitCommandBack(this.inputBuffer.GetBool(startIndex));
                case 0x82: return this.PackReadBitCommandBack(this.outputBuffer.GetBool(startIndex));
                case 0x83: return this.PackReadBitCommandBack(this.memeryBuffer.GetBool(startIndex));
                case 0x84: return this.PackReadBitCommandBack(this.dbBlockBuffer.GetBool(startIndex));
            ...
            switch (command[8]) {
                case 0x81: return this.PackReadWordCommandBack(this.inputBuffer.GetBytes(startIndex, length));
                case 0x82: return this.PackReadWordCommandBack(this.outputBuffer.GetBytes(startIndex, length));
                case 0x83: return this.PackReadWordCommandBack(this.memeryBuffer.GetBytes(startIndex, length));
                case 0x84: return this.PackReadWordCommandBack(this.dbBlockBuffer.GetBytes(startIndex, length));
                default: throw new Exception(StringResources.Language.NotSupportedDataType);
            }
        }
    }
    ...
    private byte[] PackReadWordCommandBack(byte[] result) {
        ...
        this.ByteTransform.TransByte((ushort) result.Length).CopyTo(back, 2);
    ...
    private byte[] WriteByMessage(byte[] packCommand) {
        if (packCommand[22] == 0x02) {
            // 字写入
            int count = this.ByteTransform.TransInt16(packCommand, 23);
            int startIndex = (packCommand[28] * 65536 + packCommand[29] * 256 + packCommand[30]) / 8;
            byte[] data = this.ByteTransform.TransByte(packCommand, 35, count);
            switch (packCommand[27]) {
                case 0x81:
                    this.inputBuffer.SetBytes(data, startIndex);
                    break;
                case 0x82:
                    this.outputBuffer.SetBytes(data, startIndex);
                    break;
                case 0x83:
                    this.memeryBuffer.SetBytes(data, startIndex);
                    break;
                case 0x84:
                    this.dbBlockBuffer.SetBytes(data, startIndex);
                    break;
                default: throw new Exception(StringResources.Language.NotSupportedDataType);
            }
            ...
            switch (packCommand[27]) {
                case 0x81:
                    this.inputBuffer.SetBool(value, startIndex);
                    break;
                case 0x82:
                    this.outputBuffer.SetBool(value, startIndex);
                    break;
                case 0x83:
                    this.memeryBuffer.SetBool(value, startIndex);
                    break;
                case 0x84:
                    this.dbBlockBuffer.SetBool(value, startIndex);
                    break;
                default: throw new Exception(StringResources.Language.NotSupportedDataType);
            }
    ...
    protected override void LoadFromBytes(byte[] content) {
        ...
        this.inputBuffer.SetBytes(content, 0, 0, DataPoolLength);
        this.outputBuffer.SetBytes(content, DataPoolLength, 0, DataPoolLength);
        this.memeryBuffer.SetBytes(content, DataPoolLength * 2, 0, DataPoolLength);
        this.dbBlockBuffer.SetBytes(content, DataPoolLength * 3, 0, DataPoolLength);
    }
    ...
    protected override byte[] SaveToBytes() {
        ...
        Array.Copy(this.inputBuffer.GetBytes(), 0, buffer, 0, DataPoolLength);
        Array.Copy(this.outputBuffer.GetBytes(), 0, buffer, DataPoolLength, DataPoolLength);
        Array.Copy(this.memeryBuffer.GetBytes(), 0, buffer, DataPoolLength * 2, DataPoolLength);
        Array.Copy(this.dbBlockBuffer.GetBytes(), 0, buffer, DataPoolLength * 3, DataPoolLength);
    ...
    public override string ToString() {
        return $"SiemensS7Server[{this.Port}]";
    }
}