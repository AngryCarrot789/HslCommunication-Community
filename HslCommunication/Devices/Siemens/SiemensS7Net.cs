...
/// </remarks>
/// <example>
/// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="Usage" title="简单的短连接使用" />
/// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="Usage2" title="简单的长连接使用" />
/// </example>
public class SiemensS7Net : NetworkDeviceBase<S7Message, ReverseBytesTransform> {
    ...
    public byte Slot {
        ...
        set {
            this.plc_slot = value;
            this.plcHead1[21] = (byte) ((this.plc_rack * 0x20) + this.plc_slot);
        }
    }
    ...
    protected override OperateResult InitializationOnConnect(Socket socket) {
        ...
        OperateResult<byte[]> read_first = this.ReadFromCoreServer(socket, this.plcHead1);
        if (!read_first.IsSuccess)
            return read_first;

        // 第二次握手 -> Second handshake
        OperateResult<byte[]> read_second = this.ReadFromCoreServer(socket, this.plcHead2);
    ...
    public OperateResult<string> ReadOrderNumber() {
        OperateResult<byte[]> read = this.ReadFromCoreServer(this.plcOrderNumber);
        if (!read.IsSuccess)
    ...
    public OperateResult HotStart() {
        OperateResult<byte[]> read = this.ReadFromCoreServer(this.S7_HOT_START);
        if (!read.IsSuccess)
            return read;

        if (read.Content.Length < 19)
            return new OperateResult("Receive error");

        if (read.Content[19] != pduStart)
            return new OperateResult("Can not start PLC");
        else if (read.Content[20] != pduAlreadyStarted)
            return new OperateResult("Can not start PLC");
    ...
    public OperateResult ColdStart() {
        OperateResult<byte[]> read = this.ReadFromCoreServer(this.S7_COLD_START);
        if (!read.IsSuccess)
            return read;
    ...
    public OperateResult Stop() {
        OperateResult<byte[]> read = this.ReadFromCoreServer(this.S7_STOP);
        if (!read.IsSuccess)
            return read;
        ...
        if (read.Content[19] != pduStop)
            return new OperateResult("Can not stop PLC");
        else if (read.Content[20] != pduAlreadyStopped)
            return new OperateResult("Can not stop PLC");
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="ReadExample2" title="Read示例" />
    /// 以下是读取不同类型数据的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="ReadExample1" title="Read示例" />
    /// </example>
    public override OperateResult<byte[]> Read(string address, ushort length) {
        ...
        while (alreadyFinished < length) {
            ...
            OperateResult<byte[]> read = this.Read(new S7AddressData[] { addressResult.Content });
            if (!read.IsSuccess)
                return read;
    ...
    public OperateResult<byte[]> Read(S7AddressData[] s7Addresses) {
        if (s7Addresses.Length > 19) {
            ...
            for (int i = 0; i < groups.Count; i++) {
                OperateResult<byte[]> read = this.Read(groups[i]);
                if (!read.IsSuccess)
                    return read;
    ...
    /// <example>
    /// 假设读取M100.0的位是否通断
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="ReadBool" title="ReadBool示例" />
    /// </example>
    public override OperateResult<bool> ReadBool(string address) {
        return ByteTransformHelper.GetResultFromBytes(this.ReadBitFromPLC(address), m => m[0] != 0x00);
    }
    ...
    public OperateResult<byte> ReadByte(string address) {
        return ByteTransformHelper.GetResultFromArray(this.Read(address, 1));
    }
    ...
    private OperateResult WriteBase(byte[] entireValue) {
        OperateResult<byte[]> write = this.ReadFromCoreServer(entireValue);
        if (!write.IsSuccess)
            return write;
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="WriteExample2" title="Write示例" />
    /// 以下是写入不同类型数据的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="WriteExample1" title="Write示例" />
    /// </example>
    public override OperateResult Write(string address, byte[] value) {
        ...
        while (alreadyFinished < length) {
            ...
            byte[] buffer = this.ByteTransform.TransByte(value, alreadyFinished, writeLength);

            OperateResult<byte[]> command = BuildWriteByteCommand(analysis, buffer);
            if (!command.IsSuccess)
                return command;

            OperateResult write = this.WriteBase(command.Content);
            if (!write.IsSuccess)
                return write;
    ...
    /// <example>
    /// 假设写入M100.0的位是否通断
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\SiemensS7Net.cs" region="WriteBool" title="WriteBool示例" />
    /// </example>
    public override OperateResult Write(string address, bool value) {
        ...
        return this.WriteBase(command.Content);
    }
    ...
    public override OperateResult Write(string address, string value) {
        ...
        if (this.CurrentPlc != SiemensPLCS.S200Smart) {
            // need read one time
            OperateResult<byte[]> readLength = this.Read(address, 2);
            if (!readLength.IsSuccess)
                return readLength;

            if (readLength.Content[0] == 255)
                return new OperateResult<string>("Value in plc is not string type");
            if (readLength.Content[0] == 0)
                readLength.Content[0] = 254; // allow to create new string
            if (value.Length > readLength.Content[0])
                return new OperateResult<string>("String length is too long than plc defined");

            return this.Write(address, SoftBasic.SpliceTwoByteArray(new byte[] { readLength.Content[0], (byte) buffer.Length }, buffer));
        }
        else {
            return this.Write(address, SoftBasic.SpliceTwoByteArray(new byte[] { (byte) buffer.Length }, buffer));
        }
    }
    ...
    public OperateResult<string> ReadString(string address) {
        if (this.CurrentPlc != SiemensPLCS.S200Smart) {
            ...
            if (read.Content[0] == 0 || read.Content[0] == 255)
                return new OperateResult<string>("Value in plc is not string type"); // max string length can't be zero

            OperateResult<byte[]> readString = this.Read(address, (ushort) (2 + read.Content[1]));
            ...
            OperateResult<byte[]> readString = this.Read(address, (ushort) (1 + read.Content[0]));
            if (!readString.IsSuccess)
                return OperateResult.CreateFailedResult<string>(readString);
