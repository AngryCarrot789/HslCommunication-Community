...
namespace HslCommunication.Devices.OpenProtocol;
...
public class OpenProtocolNet : NetworkDoubleBase<OpenProtocolMessage, RegularByteTransform> {
    ...
    protected override OperateResult InitializationOnConnect(Socket socket) {
        // 此处的 revison 应该等于多少？
        OperateResult<string> open = this.ReadCustomer(1, 0, 0, 0, null);
        if (!open.IsSuccess)
            return open;
    ...
    public override string ToString() {
        return $"OpenProtocolNet[{this.IpAddress}:{this.Port}]";
    }
