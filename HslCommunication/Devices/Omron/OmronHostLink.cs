...
public class OmronHostLink : SerialDeviceBase<ReverseWordTransform> {
    ...
    public override OperateResult<byte[]> Read(string address, ushort length) {
        // 解析地址
        OperateResult<byte[]> command = OmronFinsNetHelper.BuildReadCommand(address, length, false);
    ...
    public override OperateResult<bool[]> ReadBool(string address, ushort length) {
        // 获取指令
        OperateResult<byte[]> command = OmronFinsNetHelper.BuildReadCommand(address, length, true);
        if (!command.IsSuccess)
    ...
    public override OperateResult Write(string address, bool[] values) {
        // 获取指令
        OperateResult<byte[]> command = OmronFinsNetHelper.BuildWriteWordCommand(address, values.Select(m => m ? (byte) 0x01 : (byte) 0x00).ToArray(), true);
        ;
        if (!command.IsSuccess)
            return command;
    ...
    public override string ToString() {
        return $"OmronHostLink[{this.PortName}:{this.BaudRate}]";
    }
