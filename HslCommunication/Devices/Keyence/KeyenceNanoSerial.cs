...
public class KeyenceNanoSerial : SerialDeviceBase<KeyenceNanoByteTransform> {
    ...
    private OperateResult<byte[]> BuildReadCommand(string address, ushort length) {
        OperateResult<ushort> addressResult = KvCalculateWordStartAddress(address);
    ...
    public new OperateResult<short> ReadInt16(string address) {
        OperateResult<short[]> result = this.ReadInt16(address, 1);
        if (!result.IsSuccess)
    ...
    public new OperateResult<ushort> ReadUInt16(string address) {
        OperateResult<ushort[]> result = this.ReadUInt16(address, 1);
        if (!result.IsSuccess)
    ...
    public new OperateResult<int> ReadInt32(string address) {
        OperateResult<int[]> result = this.ReadInt32(address, 1);
        if (!result.IsSuccess)
    ...
    public new OperateResult<uint> ReadUInt32(string address) {
        OperateResult<uint[]> result = this.ReadUInt32(address, 1);
        if (!result.IsSuccess)
    ...
    public override OperateResult<bool[]> ReadBool(string address, ushort length) {
        string[] strBuffer = Encoding.Default.GetString(this.Read(address, length).Content).Split(' ');

        bool[] result = new bool[strBuffer.Length];
    ...
    public override OperateResult Write(string address, bool value) {
        ...
        byte[] byteTemp = value ? new byte[] { 0x20, 0x31, 0x20, 0x31 } : new byte[] { 0x20, 0x31, 0x20, 0x30 };
        // 先获取指令
        OperateResult<byte[]> command = this.BuildWriteCommand(address, byteTemp);
        if (!command.IsSuccess)
            return command;
    ...
    public override string ToString() {
        return $"KeyenceNanoSerial[{this.PortName}:{this.BaudRate}]";
    }
    ...
    private static OperateResult<ushort> KvCalculateWordStartAddress(string address) {
        // 初步解析，失败就返回
        OperateResult<KeyenceDataType, ushort> analysis = KvAnalysisAddress(address);
    ...
    private static OperateResult<KeyenceDataType, ushort> KvAnalysisAddress(string address) {
        OperateResult<KeyenceDataType, ushort> result = new OperateResult<KeyenceDataType, ushort>();
        try {
