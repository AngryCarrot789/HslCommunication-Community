...
/// </remarks>
/// <example>
/// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="Usage" title="简单的短连接使用" />
/// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="Usage2" title="简单的长连接使用" />
/// </example>
public class MelsecMcAsciiNet : NetworkDeviceBase<MelsecQnA3EAsciiMessage, RegularByteTransform> {
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="ReadExample2" title="Read示例" />
    /// 以下是读取不同类型数据的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="ReadExample1" title="Read示例" />
    /// </example>
    public override OperateResult<byte[]> Read(string address, ushort length) {
        ...
        while (alreadyFinished < length) {
            ...
            OperateResult<byte[]> read = this.ReadAddressData(addressResult.Content);
            if (!read.IsSuccess)
                return read;
    ...
    private OperateResult<byte[]> ReadAddressData(McAddressData addressData) {
        ...
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(coreResult, this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="WriteExample2" title="Write示例" />
    /// 以下是读取不同类型数据的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="WriteExample1" title="Write示例" />
    /// </example>
    /// <returns>结果</returns>
    public override OperateResult Write(string address, byte[] value) {
        // 分析地址
        OperateResult<McAddressData> addressResult = this.McAnalysisAddress(address, 0);
        if (!addressResult.IsSuccess)
            return OperateResult.CreateFailedResult<byte[]>(addressResult);
    ...
    /// </remarks>
    /// <example>
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="ReadBool" title="Bool类型示例" />
    /// </example>
    ...
    /// <example>
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Profinet\MelsecAscii.cs" region="WriteBool" title="Write示例" />
    /// </example>
    ...
    public OperateResult RemoteRun() {
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(Encoding.ASCII.GetBytes("1001000000010000"), this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
            return read;
        ...
        ushort errorCode = Convert.ToUInt16(Encoding.ASCII.GetString(read.Content, 18, 4), 16);
        if (errorCode != 0)
            return new OperateResult(errorCode, StringResources.Language.MelsecPleaseReferToManulDocument);
    ...
    public OperateResult RemoteStop() {
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(Encoding.ASCII.GetBytes("100200000001"), this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
            return read;
    ...
    public OperateResult<string> ReadPlcType() {
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(Encoding.ASCII.GetBytes("01010000"), this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
        ...
        ushort errorCode = Convert.ToUInt16(Encoding.ASCII.GetString(read.Content, 18, 4), 16);
        if (errorCode != 0)
            return new OperateResult<string>(errorCode, StringResources.Language.MelsecPleaseReferToManulDocument);
    ...
    public override string ToString() {
        return $"MelsecMcAsciiNet[{this.IpAddress}:{this.Port}]";
    }
