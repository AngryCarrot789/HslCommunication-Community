...
public class MelsecMcNet : NetworkDeviceBase<MelsecQnA3EBinaryMessage, RegularByteTransform> {
    ...
    public override OperateResult Write(string address, byte[] value) {
        ...
        return this.WriteAddressData(addressResult.Content, value);
    }
    ...
    public override OperateResult<bool[]> ReadBool(string address, ushort length) {
        ...
        // 数据解析，需要传入是否使用位的参数
        OperateResult<byte[]> extract = ExtractActualData(SoftBasic.BytesArrayRemoveBegin(read.Content, 11), true);
    ...
    public OperateResult RemoteRun() {
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(new byte[] { 0x01, 0x10, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00 }, this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
            return read;
    ...
    public OperateResult RemoteStop() {
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(new byte[] { 0x02, 0x10, 0x00, 0x00, 0x01, 0x00 }, this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
            return read;
    ...
    public OperateResult RemoteReset() {
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(new byte[] { 0x06, 0x10, 0x00, 0x00, 0x01, 0x00 }, this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
            return read;
    ...
    public OperateResult<string> ReadPlcType() {
        // 核心交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(PackMcCommand(new byte[] { 0x01, 0x01, 0x00, 0x00 }, this.NetworkNumber, this.NetworkStationNumber));
        if (!read.IsSuccess)
    ...
    public override string ToString() {
        return $"MelsecMcNet[{this.IpAddress}:{this.Port}]";
    }
