...
namespace HslCommunication.Devices.Panasonic;
...
public class PanasonicMewtocol : SerialDeviceBase<RegularByteTransform> {
    ...
    public override OperateResult<byte[]> Read(string address, ushort length) {
        // 创建指令
        OperateResult<byte[]> command = BuildReadCommand(this.Station, address, length);
    ...
    public override OperateResult<bool[]> ReadBool(string address, ushort length) {
        // 读取数据
        OperateResult<byte[]> read = this.Read(address, length);
        if (!read.IsSuccess)
    ...
    public override OperateResult Write(string address, bool[] values) {
        ...
        // 创建指令
        OperateResult<byte[]> command = BuildWriteCommand(this.Station, address, BasicFramework.SoftBasic.BytesReverseByWord(buffer), (short) values.Length);
        if (!command.IsSuccess)
            return command;
    ...
    public override string ToString() {
        return $"Panasonic Mewtocol[{this.PortName}:{this.BaudRate}]";
    }
