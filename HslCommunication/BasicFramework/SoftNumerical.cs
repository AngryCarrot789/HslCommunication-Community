...
public sealed class SoftNumericalOrder : SoftFileSaveBase {
    ...
    public SoftNumericalOrder(string textHead, string timeFormate, int numberLength, string fileSavePath) {
        this.LogHeaderText = "SoftNumericalOrder";
    ...
    public void ClearNumericalOrder() {
        Interlocked.Exchange(ref this.CurrentIndex, 0);
    ...
    public string GetNumericalOrder() {
        long number = Interlocked.Increment(ref this.CurrentIndex);
        this.AsyncCoordinator.StartOperaterInfomation();
        if (string.IsNullOrEmpty(this.TimeFormate)) {
            return this.TextHead + number.ToString().PadLeft(this.NumberLength, '0');
        }
        else {
            return this.TextHead + DateTime.Now.ToString(this.TimeFormate) + number.ToString().PadLeft(this.NumberLength, '0');
        }
    }
    ...
    public string GetNumericalOrder(string textHead) {
        ...
        this.AsyncCoordinator.StartOperaterInfomation();
        if (string.IsNullOrEmpty(this.TimeFormate)) {
            return textHead + number.ToString().PadLeft(this.NumberLength, '0');
        }
        else {
            return textHead + DateTime.Now.ToString(this.TimeFormate) + number.ToString().PadLeft(this.NumberLength, '0');
        }
    }
