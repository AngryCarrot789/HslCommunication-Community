...
/// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Modbus\ModbusAsciiExample.cs" region="Example" title="Modbus示例" />
/// 复杂的读取数据的代码示例如下：
/// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Modbus\ModbusAsciiExample.cs" region="ReadExample" title="read示例" />
/// 写入数据的代码如下：
...
public class ModbusAscii : ModbusRtu {
    ...
    protected override OperateResult<byte[]> CheckModbusTcpResponse(byte[] send) {
        ...
        // 核心交互
        OperateResult<byte[]> result = this.ReadBase(modbus_ascii);
        if (!result.IsSuccess)
            return result;
        ...
        if ((send[1] + 0x80) == modbus_core.Content[1])
            return new OperateResult<byte[]>(modbus_core.Content[2], ModbusInfo.GetDescriptionByErrorCode(modbus_core.Content[2]));
