...
public class ModbusTcpNet : NetworkDeviceBase<ModbusTcpMessage, ReverseWordTransform> {
    ...
    public OperateResult<byte[]> BuildReadInputRegisterCommand(string address, ushort length) {
        // 解析富地址
        OperateResult<ModbusAddress> analysis = ModbusInfo.AnalysisAddress(address, this.isAddressStartWithZero, ModbusInfo.ReadInputRegister);
        if (!analysis.IsSuccess)
            return OperateResult.CreateFailedResult<byte[]>(analysis);
    ...
    private OperateResult<byte[]> CheckModbusTcpResponse(byte[] send) {
        OperateResult<byte[]> resultBytes = this.ReadFromCoreServer(send);
    ...
    private OperateResult<byte[]> ReadModBusBase(byte code, string address, ushort length) {
        ...
        switch (code) {
            ...
            default:
                command = new OperateResult<byte[]>(StringResources.Language.ModbusTcpFunctionCodeNotSupport);
                break;
        }
    ...
    /// <example>
    /// 此处演示批量读取的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Modbus\Modbus.cs" region="ReadExample1" title="Read示例" />
    /// </example>
    ...
    /// <example>
    /// 此处演示批量写入的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Modbus\Modbus.cs" region="WriteExample1" title="Write示例" />
    /// </example>
