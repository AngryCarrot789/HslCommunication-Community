...
public class HslReflectionHelper {
    ...
    public static void SetPropertyExp<T, K>(PropertyInfo propertyInfo, T obj, K objValue) {
        ...
        ParameterExpression invokeObjExpr = Expression.Parameter(typeof(T), "obj");
        ParameterExpression propValExpr = Expression.Parameter(propertyInfo.PropertyType, "objValue");
        MethodCallExpression setMethodExp = Expression.Call(invokeObjExpr, propertyInfo.GetSetMethod(), propValExpr);
        Expression<Action<T, K>> lambda = Expression.Lambda<Action<T, K>>(setMethodExp, invokeObjExpr, propValExpr);
