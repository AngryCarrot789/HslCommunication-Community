...
internal sealed class AsyncCoordinator {
    ...
    public void AllBegun(Action<CoordinationStatus> callback, int timeout = Timeout.Infinite) {
        ...
        if (timeout != Timeout.Infinite) {
            this.m_timer = new Timer(this.TimeExpired, null, timeout, Timeout.Infinite);
        }

        this.JustEnded(); //修正一开始设置的初始值
    }
...
public sealed class HslAsyncCoordinator {
    ...
    private void ThreadPoolOperater(object obj) {
        long currentVal = this.Target, startVal;
        long desiredVal = 0;
        do {
            startVal = currentVal; //设置值
            // 以下为业务逻辑，允许实现非常复杂的设置
            this.action?.Invoke();
            // 需要清零值的时候必须用下面的原子操作
            currentVal = Interlocked.CompareExchange(ref this.Target, desiredVal, startVal);
        } while (startVal != currentVal); // 更改失败就强制更新

        // 退出保存状态
        Interlocked.Exchange(ref this.OperaterStatus, 0);
        // 最终状态确认
        if (this.Target != desiredVal)
...
public sealed class HslReadWriteLock : IDisposable {
    ...
    public void Leave() {
        ...
        if (this.m_exclusive) {
            ...
            wakeup = DoneWriting(ref this.m_LockState);
        }
        else {
            OneManyLockStates s = State(this.m_LockState);
            ...
            // Phase 1: Release the lock
            wakeup = DoneReading(ref this.m_LockState);
        }
        ...
        if (wakeup == -1)
            ...
            this.m_ReadersLock.Release(wakeup);
    }
    ...
    private void ThreadBackground() {
        while (true) {
            ...
            if (index < 0) {
                ...
                T item = this.m_dataList[index];
                bool result = false;
                bool isException = false;
                try {
                    if (!this.m_isQuit)
                        result = this.m_operater(item);
                }
                catch (Exception ex) {
                    isException = true;
                    // 此处必须吞噬所有异常
                    this.OnExceptionOccur?.Invoke(item, ex);
