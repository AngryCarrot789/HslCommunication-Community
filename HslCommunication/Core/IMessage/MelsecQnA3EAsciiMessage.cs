...
public class MelsecQnA3EAsciiMessage : INetMessage {
    ...
    public int GetContentLengthByHeadBytes() {
        ...
        buffer[0] = this.HeadBytes[14];
        buffer[1] = this.HeadBytes[15];
        buffer[2] = this.HeadBytes[16];
        buffer[3] = this.HeadBytes[17];
    ...
    public bool CheckHeadBytesLegal(byte[] token) {
        ...
        if (this.HeadBytes[0] == (byte) 'D' && this.HeadBytes[1] == (byte) '0' && this.HeadBytes[2] == (byte) '0' && this.HeadBytes[3] == (byte) '0') {
            return true;
        }
        else {
            return false;
