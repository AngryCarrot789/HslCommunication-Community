...
public class HslMessage : INetMessage {
    ...
    public bool CheckHeadBytesLegal(byte[] token) {
        ...
        if (this.HeadBytes?.Length >= 32) {
            return BasicFramework.SoftBasic.IsTwoBytesEquel(this.HeadBytes, 12, token, 0, 16);
        }
        else {
            return false;
        }
    ...
    public int GetContentLengthByHeadBytes() {
        if (this.HeadBytes?.Length >= 32) {
            return BitConverter.ToInt32(this.HeadBytes, 28);
        }
        else {
            return 0;
        }
    ...
    public int GetHeadBytesIdentity() {
        if (this.HeadBytes?.Length >= 32) {
            return BitConverter.ToInt32(this.HeadBytes, 4);
        }
        else {
            return 0;
        }
