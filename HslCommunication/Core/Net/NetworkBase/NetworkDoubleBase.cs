...
public class NetworkDoubleBase<TNetMessage, TTransform> : NetworkBase, IDisposable where TNetMessage : INetMessage, new() where TTransform : IByteTransform, new() {
    ...
    public NetworkDoubleBase() {
        this.ByteTransform = new TTransform(); // 实例化变换类的对象
    ...
    /// <example>
    /// 设置1秒的超时的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="ConnectTimeOutExample" title="ConnectTimeOut示例" />
    /// </example>
    /// <remarks>
    /// 不适用于异形模式的连接。
    /// </remarks>
    ...
    /// <example>
    /// 设置1秒的接收超时的示例
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="ReceiveTimeOutExample" title="ReceiveTimeOut示例" />
    /// </example>
    /// <remarks>
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="IpAddressExample" title="IpAddress示例" />
    /// </example>
    ...
    /// <example>
    /// 以下的方式演示了另一种长连接的机制
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="SetPersistentConnectionExample" title="SetPersistentConnection示例" />
    /// </example>
    ...
    ///   <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="Connect1" title="连接设备" />
    ///   如果想知道是否连接成功，请参照下面的代码。
    ///   <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="Connect2" title="判断连接结果" />
    /// </example> 
    public OperateResult ConnectServer() {
        ...
        OperateResult<Socket> rSocket = this.CreateSocketAndInitialication();
    ...
    /// <example>
    ///   简单的创建示例。
    ///   <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="AlienConnect1" title="连接设备" />
    ///   如果想知道是否创建成功。通常都是成功。
    ///   <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="AlienConnect2" title="判断连接结果" />
    /// </example> 
    /// <remarks>
    ...
    public OperateResult ConnectServer(AlienSession session) {
        ...
        if (session != null) {
            ...
            if (this.ConnectionId == session.DTU) {
                ...
                return this.InitializationOnConnect(session.Socket);
            }
            else {
                this.IsSocketError = true;
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="ConnectCloseExample" title="关闭连接结果" />
    /// </example>
    ...
    protected OperateResult AccountCertificate(Socket socket) {
        ...
        OperateResult<int, string[]> read = this.ReceiveStringArrayContentFromSocket(socket);
        if (!read.IsSuccess)
            return read;
        ...
        if (this.isPersistentConn) {
            ...
            if (this.isUseSpecifiedSocket) {
                if (this.IsSocketError) {
                    ...
                    return OperateResult.CreateSuccessResult(this.CoreSocket);
                }
            }
            else {
                // 长连接模式
    ...
    private OperateResult<Socket> CreateSocketAndInitialication() {
        OperateResult<Socket> result = this.CreateSocketAndConnect(new IPEndPoint(IPAddress.Parse(this.ipAddress), this.port), this.connectTimeOut);
        if (result.IsSuccess) {
            // 初始化
            OperateResult initi = this.InitializationOnConnect(result.Content);
            if (!initi.IsSuccess) {
    ...
    public virtual OperateResult<byte[]> ReadFromCoreServer(Socket socket, byte[] send) {
        ...
        // check
        if (!netMessage.CheckHeadBytesLegal(this.Token.ToByteArray())) {
            socket?.Close();
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetworkDoubleBase.cs" region="ReadFromCoreServerExample2" title="ReadFromCoreServer示例" />
    /// </example>
    public OperateResult<byte[]> ReadFromCoreServer(byte[] send) {
        ...
        OperateResult<byte[]> read = this.ReadFromCoreServer(resultSocket.Content, send);
