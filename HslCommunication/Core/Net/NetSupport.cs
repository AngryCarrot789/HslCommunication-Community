...
/// <example>
/// 一个接收的示例
/// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetSupport.cs" region="ReadBytesFromSocketExample2" title="ReadBytesFromSocket示例" />
/// </example>
public static class NetSupport {
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetSupport.cs" region="ReadBytesFromSocketExample1" title="ReadBytesFromSocket示例" />
    /// 如何接收不定长度的数据呢？我们可以将一条数据拆分成2次接收，第一次是接收8个固定的字节，解析成长度，再接收真实的数据。
    ///  <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetSupport.cs" region="ReadBytesFromSocketExample3" title="ReadBytesFromSocket示例" />
    /// </example>
    ...
    public static byte[] ReadBytesFromSocket(Socket socket, int receive, Action<long, long> report, bool reportByPercent, bool response) {
        ...
        while (count_receive < receive) {
            ...
            // 回发进度
            if (response)
                socket.Send(BitConverter.GetBytes((long) count_receive));
        }
    ...
    public static OperateResult<byte[]> ReceiveCommandLineFromSocket(Socket socket, byte endCode) {
        ...
        try {
            // 接收到endCode为止
            while (true) {
                byte[] head = ReadBytesFromSocket(socket, 1);
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetSupport.cs" region="WriteStreamFromSocketExample" title="WriteStreamFromSocket示例" />
    /// </example>
    ...
    /// <code lang="cs" source="HslCommunication.Test\Documentation\Samples\Core\NetSupport.cs" region="WriteSocketFromStreamExample" title="WriteSocketFromStream示例" />
    /// </example>
