...
class FP1_C72 {
    ...
    private string tobcc(string s) //帧校验函数FCS
    {
        ...
        return this.intToHex(t / 16) + this.intToHex(t % 16);
    }
    ...
    public bool PortOpen(int port, int baudrate, int databit, int stopbit, int oddEven, ref string errMsg) {
        this.serialPort1.PortName = "COM" + port;
        ...
        switch (stopbit) {
            case 0:
                this.serialPort1.StopBits = StopBits.None;
                break;
            case 1:
        ...
        this.serialPort1.DataBits = databit;
        try {
            this.serialPort1.Open();
            if (!this.SendPlcCmd("%EE#RT", ref rets, ref errs)) {
    ...
    private string GetPlcErrInfo(int errcode) {
        switch (errcode) {
            case 21: return "NACK 错误:远程单元无法被正确识别，或者发生了数据错误.";
            case 22: return "WACK 错误: 用于远程单元的接收缓冲区已满. ";
            case 23: return "多重端口错误:远程单元编号(01 至16)设置与本地单元重复. ";
            case 24: return "传输格式错误:试图发送不符合传输格式的数据或者某一帧数据溢出或发生了数据错误. ";
            case 25: return "硬件错误: 传输系统硬件停止操作.";
            case 26: return "单元号错误: 远程单元的编号设置超出01 至63 的范围. ";
            case 27: return "不支持错误: 接收方数据帧溢出. 试图在不同的模块之间发送不同帧长度的数据.";
            case 28: return "无应答错误:远程单元不存在(超时) .";
            case 29: return "缓冲区关闭错误:试图发送或接收处于关闭状态的缓冲区,持续处于传输禁止状态.";
            case 30: return "超时错误: ";
            case 40: return "BCC 错误: 在指令数据中发生传输错误. ";
            case 41: return "格式错误:所发送的指令信息不符合传输格式. 例: 指令中的数据项目过多或不足. 缺少'#'符号及'目标站号' .";
            case 42: return "不支持错误: 发送了一个未被支持的指令. 向未被支持的目标站发送了指令. ";
            case 43: return "处理步骤错误:在处于传输请求信息挂起时,发送了其他指令. ";
            case 50: return "链接设置错误: 设置了实际不存在的链接编号. ";
            case 51: return "同时操作错误:当向其他单元发出指令时,本地单元的传输缓冲区已满 .";
            case 52: return "传输禁止错误: 无法向其他单元传输. ";
            case 53: return "忙错误:在接收到指令时,正在处理其他指令. ";
            case 60: return " 参数错误: 在指令中包含有无法使用的代码,或者代码没有附带区域指定参数(X,Y,D, 等以外.)指令中的代码带有非法的功能指定参数(0,1,2,等). ";
            case 61: return "数据错误: 触点编号,区域编号,数据代码格式(BCD,hex等)上溢出, 下溢出以及区域指定错误.";
            case 62: return "寄存器错误: 过多记录数据在未记录状态下的操作（监控记录、跟踪记录等。) 当记录溢出时，将进行重新记录。";
            case 63: return "PLC 模式错误: 当一条指令发出时，运行模式不能够对指令进行处理。";
            case 65: return "保护错误: 在存储保护状态下执行写操作到程序区域或系统寄存器。";
            case 66: return "地址错误: 地址（程序地址、绝对地址等）数据编码形式（BCD、hex 等）、上溢、下溢或指定范围错误。";
            case 67: return "丢失数据错误: 要读的数据不存在。（ 读取没有写入注释寄存区的数据。）";
            case 20:
            default:
                return "未定义.";
        }
    ...
    private bool SendPlcCmd(string cmdstr, ref string retStr, ref string errStr) {
        ...
        try {
            int DataLength = this.serialPort1.BytesToRead;
            if (DataLength > 0)
                this.serialPort1.DiscardInBuffer();
            this.serialPort1.Write(cmdstr + this.tobcc(cmdstr) + (char) 13);
            Thread.Sleep(100);
            DataLength = this.serialPort1.BytesToRead;
            if (DataLength > 0) {
                StringBuilder sb = new StringBuilder();
                byte[] ds = new byte[DataLength + 1];
                while (i < DataLength) {
                    int len = this.serialPort1.Read(ds, 0, DataLength + 1);
                ...
                if (ms.Length > 5) {
                    if (ms.Substring(0, 1) == "%" && ms.Substring(3, 1) == "!") {
                        i = Convert.ToInt16(ms.Substring(4, 2));
                        errStr = this.GetPlcErrInfo(i);
                        return false;
                    }
                }
            }
    ...
    public bool ReadContactPoint(char code, string address, ref string retstr, ref string errstr) {
        ...
        return this.SendPlcCmd(sendstr, ref retstr, ref errstr);
    }
    ...
    public bool MonitorContactPoint(string addressM, ref string retstr, ref string errstr) {
        ...
        sendstr = "%EE#MCFFFFF";
        if (!this.SendPlcCmd(sendstr, ref rets, ref errs)) {
            errstr = errs;
            retstr = rets;
            return false;
        }
