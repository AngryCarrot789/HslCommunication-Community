...
/// 注意：M地址范围有限 0-31地址
/// </remarks>
public class SiemensPPI : SerialDeviceBase<Core.ReverseBytesTransform> {
    #region Constructor
    ...
    public byte Station {
        ...
        set {
            ...
            int count = 0;
            for (int i = 1; i < 4; i++) {
                count += this.executeConfirm[i];
            }

            this.executeConfirm[4] = (byte) count;
        }
    }
    ...
    public override OperateResult<byte[]> Read(string address, ushort length) {
        ...
        if (read2.Content.Length < 21)
            return new OperateResult<byte[]>(read2.ErrorCode, "Failed: " + BasicFramework.SoftBasic.ByteToHexString(read2.Content, ' '));
        if (read2.Content[17] != 0x00 || read2.Content[18] != 0x00)
            return new OperateResult<byte[]>(read2.Content[19], GetMsgFromStatus(read2.Content[18], read2.Content[19]));
    ...
    public override OperateResult<bool[]> ReadBool(string address, ushort length) {
        ...
        // 错误码判断
        if (read2.Content.Length < 21)
            return new OperateResult<bool[]>(read2.ErrorCode, "Failed: " + BasicFramework.SoftBasic.ByteToHexString(read2.Content, ' '));
        if (read2.Content[17] != 0x00 || read2.Content[18] != 0x00)
            return new OperateResult<bool[]>(read2.Content[19], GetMsgFromStatus(read2.Content[18], read2.Content[19]));
        if (read2.Content[21] != 0xFF)
    ...
    public override OperateResult Write(string address, byte[] value) {
        // 解析指令
        OperateResult<byte[]> command = BuildWriteCommand(this.station, address, value);
        if (!command.IsSuccess)
            return command;
        ...
        // 错误码判断
        if (read2.Content.Length < 21)
            return new OperateResult(read2.ErrorCode, "Failed: " + BasicFramework.SoftBasic.ByteToHexString(read2.Content, ' '));
        if (read2.Content[17] != 0x00 || read2.Content[18] != 0x00)
    ...
    public override string ToString() {
        return $"SiemensPPI[{this.PortName}:{this.BaudRate}]";
    }
