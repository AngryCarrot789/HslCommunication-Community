...
public class NetSimplifyClient : NetworkDoubleBase<HslMessage, RegularByteTransform> {
    ...
    protected override OperateResult InitializationOnConnect(Socket socket) {
        if (this.isUseAccountCertificate) {
            return this.AccountCertificate(socket);
        }
    ...
    public OperateResult<string> ReadFromServer(NetHandle customer, string send) {
        OperateResult<byte[]> read = this.ReadFromServerBase(HslProtocol.CommandBytes(customer, this.Token, send));
        if (!read.IsSuccess)
    ...
    public OperateResult<byte[]> ReadFromServer(NetHandle customer, byte[] send) {
        return this.ReadFromServerBase(HslProtocol.CommandBytes(customer, this.Token, send));
    }
    ...
    public OperateResult<NetHandle, string> ReadCustomerFromServer(NetHandle customer, string send) {
        OperateResult<NetHandle, byte[]> read = this.ReadCustomerFromServerBase(HslProtocol.CommandBytes(customer, this.Token, send));
        if (!read.IsSuccess)
    ...
    public OperateResult<NetHandle, byte[]> ReadCustomerFromServer(NetHandle customer, byte[] send) {
        return this.ReadCustomerFromServerBase(HslProtocol.CommandBytes(customer, this.Token, send));
    }
    ...
    private OperateResult<byte[]> ReadFromServerBase(byte[] send) {
        OperateResult<NetHandle, byte[]> read = this.ReadCustomerFromServerBase(send);
        if (!read.IsSuccess)
    ...
    private OperateResult<NetHandle, byte[]> ReadCustomerFromServerBase(byte[] send) {
        // 核心数据交互
        OperateResult<byte[]> read = this.ReadFromCoreServer(send);
        if (!read.IsSuccess)
    ...
    public Task<OperateResult<string>> ReadFromServerAsync(NetHandle customer, string send) {
        return Task.Run(() => this.ReadFromServer(customer, send));
    }
    ...
    public Task<OperateResult<string[]>> ReadFromServerAsync(NetHandle customer, string[] sends) {
        return Task.Run(() => this.ReadFromServer(customer, sends));
    }
    ...
    public Task<OperateResult<NetHandle, string>> ReadCustomerFromServerAsync(NetHandle customer, string send) {
        return Task.Run(() => this.ReadCustomerFromServer(customer, send));
    }
