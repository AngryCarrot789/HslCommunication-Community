...
public abstract class FileClientBase : NetworkXBase {
    ...
    protected OperateResult SendFactoryGroupId(
        ...
        string group,
        string id) {
        OperateResult factoryResult = this.SendStringAndCheckReceive(socket, 1, factory);
        if (!factoryResult.IsSuccess) {
            return factoryResult;
        }

        OperateResult groupResult = this.SendStringAndCheckReceive(socket, 2, group);
        if (!groupResult.IsSuccess) {
            return groupResult;
        }

        OperateResult idResult = this.SendStringAndCheckReceive(socket, 3, id);
    ...
    protected OperateResult DeleteFileBase(string fileName, string factory, string group, string id) {
        ...
        // 发送操作指令
        OperateResult sendString = this.SendStringAndCheckReceive(socketResult.Content, HslProtocol.ProtocolFileDelete, fileName);
        if (!sendString.IsSuccess)
            return sendString;

        // 发送文件名以及三级分类信息
        OperateResult sendFileInfo = this.SendFactoryGroupId(socketResult.Content, factory, group, id);
        if (!sendFileInfo.IsSuccess)
            return sendFileInfo;

        // 接收服务器操作结果
        OperateResult<int, string> receiveBack = this.ReceiveStringContentFromSocket(socketResult.Content);
    ...
    protected OperateResult DownloadFileBase(
        ...
        // 发送操作指令
        OperateResult sendString = this.SendStringAndCheckReceive(socketResult.Content, HslProtocol.ProtocolFileDownload, fileName);
        if (!sendString.IsSuccess)
            return sendString;
        ...
        if (source is string fileSaveName) {
            OperateResult result = this.ReceiveFileFromSocket(socketResult.Content, fileSaveName, processReport);
            if (!result.IsSuccess)
                return result;
        }
        else if (source is Stream stream) {
            OperateResult result = this.ReceiveFileFromSocket(socketResult.Content, stream, processReport);
            if (!result.IsSuccess) {
                return result;
            }
        }
    ...
    protected OperateResult UploadFileBase(
        ...
        // 上传操作暗号的文件名
        OperateResult sendString = this.SendStringAndCheckReceive(socketResult.Content, HslProtocol.ProtocolFileUpload, serverName);
        if (!sendString.IsSuccess)
            return sendString;
        ...
        if (source is string fileName) {
            OperateResult result = this.SendFileAndCheckReceive(socketResult.Content, fileName, serverName, fileTag, fileUpload, processReport);
            if (!result.IsSuccess) {
                return result;
            }
        }
        else if (source is Stream stream) {
            OperateResult result = this.SendFileAndCheckReceive(socketResult.Content, stream, serverName, fileTag, fileUpload, processReport);
            if (!result.IsSuccess) {
                return result;
            }
        }
            ...
            socketResult.Content?.Close();
            this.LogNet?.WriteError(this.ToString(), StringResources.Language.DataSourseFormatError);
            return new OperateResult(StringResources.Language.DataSourseFormatError);
        }


        // 确认服务器文件保存状态
        OperateResult<int, string> resultCheck = this.ReceiveStringContentFromSocket(socketResult.Content);
